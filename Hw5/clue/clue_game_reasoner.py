...
class ClueGameReasoner:
    ...
    def __init__(self, player_order, card_nums = None):
        ...
        for p1 in SUSPECTS:
                ...
                for p2 in SUSPECTS:
                    if p1!=p2:
                        clause = "~"+p1 + "_" + CASE_FILE + " "+"~"+p2 + "_" + CASE_FILE
                        clauses.append(clause)
    ...
    def suggest(self, suggester, c1, c2, c3, refuter, cardshown = None):
        ...
        if refuter==None:
            ...
            pos=self.players.index(suggester)
            pos+=1
            while(self.players[pos%length]!=refuter):
                clause="~"+c1+ "_"+self.players[pos%length]
                clauses.append(clause)
                clause="~"+c2+ "_"+self.players[pos%length]
                clauses.append(clause)
                clause="~"+c3+ "_"+self.players[pos%length]
                clauses.append(clause)
                pos+=1
    ...
    def accuse(self, accuser, c1, c2, c3, iscorrect):
        ...
        clauses=[]
        if iscorrect:
            clause=c1+"_"+CASE_FILE
            clauses.append(clause)
            clause=c2+"_"+CASE_FILE
            clauses.append(clause)
            clause=c3+"_"+CASE_FILE
            clauses.append(clause)
        else:
            clause="~"+c1+"_"+CASE_FILE+" "
            clause+="~"+c2+"_"+CASE_FILE+" "
